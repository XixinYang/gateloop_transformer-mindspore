...
# will be adapted to test out GateLoop on rotary_embedding small scale https://arxiv.org/abs/2311.01927
...
def associative_scan(operator: Callable, elems: Tuple[Tensor, Tensor]):
    ...
    def _scan(elems):
        ...
        # The first element of rotary_embedding scan is the same as the first element
        # of the original `elems`.

        even_elems = [ops.cat([elem[:, :1], result], axis=1) for (elem, result) in zip(elems, even_elems)]
...
def _interleave(a, b):
    ...
    stacked = ops.stack([a, b], axis=2)
    interleaved = ops.flatten(stacked, start_dim=1, end_dim=2)
